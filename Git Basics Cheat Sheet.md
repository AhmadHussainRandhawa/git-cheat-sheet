...
- `git branch -v`	 # Shows the local branches name, commit hash, commit message 
- `git branch -vv`   # Additional details like whether local is ahead, behind or up-to-date with remote.

- `git fetch <remote_name>`			    	# Download updates from remote (does NOT merge)
...
- `git push -u origin <branch-name>`        # Push to remote repository & Setup an upstream remote
...
- `git stash`                    #  Stash current changes (temporary save)
...
- `git stash apply stash@{2}`		    # Apply the specific stash.
- `git stash apply stash@{2} <b-name>`  # Apply stash to a specific branch
...
## diff: Shows differences between Git states (working directory, staging area, commits, branches).

- `git diff`                    # Shows unstaged changes
- `git diff --staged`           # Shows staged changes (staging area → last commit)
- `git diff HEAD`               # Shows all changes since last commit (staged + unstaged)

- `git diff branch1..branch2`   # Shows differences between two branches (use before merging)
- `git diff commit1 commit2`    # Shows differences between two commits
- `git diff <commit> <file>`    # Shows how a specific file changed in a commit
- `git diff --stat`             # Shows a summary of changes (files changed + lines added/removed)
...
- `git reset --hard <commit-hash>`      # Undo last commit, remove all the changes even if it is staged.
...
- `git checkout  <file_name>`                          # Restore to the previous commit 
- `git checkout <commit_hash> -- <file_name>`          # Restore from a specific commit version
...
- `git rm --cached <f_name>`    # Unstage the file.
